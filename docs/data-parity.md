...
| Auth state | `AuthProvider` + Supabase SecureStore adapter | OAuth `redirectTo` from [`lib/auth/oauth-redirect.ts`](lib/auth/oauth-redirect.ts): **`splittheg://auth/callback`** on dev client / standalone; **Expo Go** uses `exp://…` (Supabase GoTrue rejects non-loopback IP hosts before the allowlist — tunnel, simulator rewrite, or dev build; see README). |
